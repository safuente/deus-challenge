...
class ContractService(BaseService):
    ...
    def create_item(self, item_data: BaseModel) -> Contract:
        ...
        if client_id is not None:
            client: Client | None = self.db.query(Client).filter(Client.client_id == client_id).first()
            if not client:
                raise HTTPException(status_code=400, detail=f"Client ID {client_id} does not exist")
