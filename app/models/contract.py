...
class Contract(Base):
    ...
    destination: str = Column(
        String, nullable=False, doc="Destination of the shipment."
    )
    price: float = Column(Float, nullable=False, doc="Price of the contract.")
    start_date: datetime = Column(
        DateTime, default=datetime.utcnow, doc="Start date of the contract."
    )
    end_date: datetime | None = Column(
        DateTime, nullable=True, doc="End date of the contract (optional)."
    )
    status: str = Column(
        String, default="active", doc="Current status of the contract."
    )

    client = relationship(
        "Client",
        back_populates="contracts",
        doc="Client associated with this contract.",
    )
    cargo = relationship(
        "Cargo",
        uselist=False,
        back_populates="contract",
        doc="Cargo associated with this contract.",
    )
