...
class TestTracking:
    def test_create_tracking_with_invalid_cargo_or_vessel(self, client: TestClient) -> None:
        ...
        assert "Cargo ID 999 does not exist" in response.json()["detail"] or "Vessel ID 999 does not exist" in response.json()["detail"]

    def test_create_tracking_with_valid_cargo_and_vessel(self, client: TestClient) -> None:
        ...
        tracking_response = client.post("/tracking/", json={
            "cargo_id": cargo_id,
            "vessel_id": vessel_id,
            "location": "Port of Hamburg",
            "status": "In Transit"
